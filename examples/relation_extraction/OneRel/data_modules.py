...
from scheme import OneRelScheme
...
scheme = OneRelScheme(max_num_tokens=max_num_tokens, num_relations=len(relation_labels))
...
class DataCollate:
    ...
    def __call__(self, batch: List[Dict[str, List[Any]]]) -> Dict[str, Tensor]:
        ...
        for sample in batch:
            ...
            target.append(self.scheme.encode(sro_set))
            sro_sets.append(sro_set)

        target = torch.stack(target, dim=0)  # [batch_size, num_relations, num_subjects, num_objects]

        ignored_mask = (input_ids == 0).unsqueeze(-1).expand(-1, -1, self.max_num_tokens)
        target = target.permute(0, 2, 3, 1)  # [batch_size, num_subjects, num_objects, num_relations]
        target[ignored_mask] = -100
        target = target.permute(0, 3, 1, 2)  # [batch_size, num_relations, num_subjects, num_objects]

        ret = {"input_ids": input_ids, "target": target}
